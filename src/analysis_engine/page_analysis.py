...
class PageAnalyzer:
    ...
    def analyze_document_pages(self, doc_id: str, fast_mode: bool = False) -> None:
        """
        Public wrapper for document analysis.
        Analyzes all pages in a document sequentially.
        """
        return self._analyze_document(doc_id, fast_mode=fast_mode)

    def _analyze_document(self, doc_id: str, fast_mode: bool = False) -> None:
        """
        ...
        for page in pages:
            self._analyze_single_page(page, pages)
    ...
    def _analyze_single_page(self, page: Dict[str, Any], all_pages: List[Dict[str, Any]]):
        """
        ...
        try:
            ...
            if len(current_text.strip()) < 50 and not vision_summary:
                print(f"⚠️  [ANALYSIS] Page {page_idx} skipped (No Signal)")
                result = {
            ...
            print("\n" + "=" * 80)
            print(f"📥 [ANALYSIS] Page {page_idx} (⏱️ {elapsed:.2f}s | Profile: {result.get('analyst_profile')})")
            print("=" * 80)
    ...
    def _save_result(
        ...
        try:
            page_rec = PageRecord(
                ...
                page_entities=json.dumps(entities),
        ...
        try:
            ...
            for ent_text in entities:
                self.db.upsert_entity_node(project_id, doc_id, EntityType.ENTITY, ent_text)
