...
EARLY_DOCUMENT_BUILD_PRIORITY = 60
...
class ExtractJob(Job):
        ...
        try:
            ...
            with db.transaction():
                for rec in result.records:
                    self._insert_record(db, rec, doc_id)
            ...
            # PDF fact build + analysis trigger
            if self.extractor_name == "pdf":
                ...
                build_job = BuildFactsJob(
                    ...
                    snapshot_id=self.file_version_id,
                    priority=EARLY_DOCUMENT_BUILD_PRIORITY,
                )
                context["manager"].submit(build_job)
                logger.info(f"Triggered early BuildFactsJob (document) for snapshot={self.file_version_id}")
    ...
    def _insert_record(self, db, rec, doc_id: str):
        rtype = rec["type"]
        data = rec["data"]
        ...
        if rtype.startswith("p6_"):
            if rtype == "p6_project":
                 ...
                 total_float = data.get("total_float")
                 if total_float in (None, ""):
                     float_hours = data.get("total_float_hours") or data.get("total_float_hr_cnt")
                     try:
                         if float_hours not in (None, ""):
                             total_float = round(float(float_hours) / 8.0, 2)
                     except (TypeError, ValueError):
                         total_float = None
                 db.execute("INSERT OR REPLACE INTO p6_activities (activity_id, file_version_id, p6_project_id, wbs_id, code, name, start_date, finish_date, status_code, total_float, raw_json) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (data.get("task_id"), vid, data.get("proj_id"), data.get("wbs_id"), data.get("task_code"), data.get("task_name"), data.get("target_start_date"), data.get("target_end_date"), data.get("status_code"), total_float, json.dumps(data)))
        ...
        # PDF Logic (NEW)
            ...
            # Bridge to V01 Page Analysis Layer 
            # This ensures 'PageAnalyzer' (VLM-based) can find the pages to process.
            from src.domain.models.page_record import PageRecord
            page_rec = PageRecord(
                doc_id=doc_id,
                page_index=data["page_no"] - 1,
                py_text=data["text_content"],
                py_text_len=len(data["text_content"]),
                py_text_extracted=True,
                updated=db._ts()
            )
            db.upsert_page(page_rec)
