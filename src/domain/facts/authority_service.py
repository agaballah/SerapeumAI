...
from src.domain.facts.models import FactStatus, TRUSTED_FACT_STATUSES
...
class AuthorityService:
    ...
    def authorize_certificate(self, fact_id: str, role_id: str, cert_type: str = FactStatus.HUMAN_CERTIFIED.value) -> bool:
        """
        Attempts to certify a fact. Checks authority first.
        Only canonical trusted states may be written by this path.
        """
        if cert_type not in TRUSTED_FACT_STATUSES:
            logger.warning("Refusing non-trusted certification target: %s", cert_type)
            return False
