...
Supports:
    • LM Studio publish runtime via local REST/OpenAI-compatible API
...
class LLMService:
    ...
    def verify_runtime_ready(self, task_type: str = "analysis", model: Optional[str] = None) -> Dict[str, Any]:
        ...
        contract = self.lm_studio.verify_chat_runtime(
    ...
    def _chat_lm_studio(
        ...
        **extra,
        ...
        # Validate runtime contract first, then load only if native model management is available.
        contract = self.lm_studio.verify_chat_runtime(
            target_model=chosen_model,
            require_stateful=False,
        ) if self.lm_studio else {"resolved_model": chosen_model}
        chosen_model = contract.get("resolved_model") or chosen_model
        ...
        try:
            ...
            if chosen_model:
                if not status.get("loaded"):
                    raise RuntimeError(
                        f"Selected model '{chosen_model}' is not loaded. Open Re-check Runtime and load the selected session model."
                    )
                def _norm(value: str) -> str:
                    return str(value or "").strip().lower().replace("\\", "/")
                chosen_norm = _norm(chosen_model)
                ...
                current_equivalents = {
                    current_norm,
                    current_norm.split("/")[-1],
                    current_norm.split(":")[0],
                }
                if current_norm and comparable.isdisjoint(current_equivalents):
                    raise RuntimeError(
                        f"Selected model '{chosen_model}' is not the active LM Studio model. Current active model: '{current_model}'. Use Re-check Runtime to load the selected model."
                    )
        except Exception as e:
            logger.error(f"[LLMService] LM Studio runtime is not ready for task '{task_type}': {e}")
            raise
