...
class ModelRouter:
    ...
    def _task_family_recommendations(self, task_type: str):
    ...
    def _recommended_model(self, task_type: str):
        for candidate in self._task_family_recommendations(task_type):
            resolved = self._resolve_to_installed_id(candidate)
            if resolved and self._is_task_capable(resolved, self._norm_task(task_type)):
                return resolved
        return None

    def _publish_locked_model(self, task: str) -> Optional[str]:
        if task not in PUBLISH_SINGLE_MODEL_TASKS:
            return None
        resolved = self._resolve_to_installed_id(PUBLISH_GENERATIVE_MODEL)
        if resolved and self._is_task_capable(resolved, task):
            return resolved
        return None
    ...
    def get_best_model(self, task_type: str, is_explicit_request: bool = False) -> str:
        ...
        # 6) built-in soft recommendations (only if stronger signals absent)
        recommended = self._recommended_model(task)
        if recommended:
            self._cache_set(task, recommended)
            return recommended

        # 7) fallback (Safe refusal if nothing fit)
