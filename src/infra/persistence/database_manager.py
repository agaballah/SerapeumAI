...
class DatabaseManager:
    ...
    def get_or_create_snapshot(self, project_id: str, label: str = "Live Snapshot") -> str:
        """
        Phase 2.3: Snapshot-to-Fact Immutability.
        Creates a persistent snapshot and locks the current certified facts.
        """
        import uuid
        ...
        self._exec(
            "INSERT INTO fact_snapshots (snapshot_id, project_id, label, created_at) VALUES (?, ?, ?, ?)",
            (snapshot_id, project_id, label, now)
        )
        
        # 2. Lock current certified facts
        self._exec(
            """
            INSERT INTO fact_snapshot_registry (snapshot_id, fact_id)
            SELECT ?, fact_id FROM facts
            WHERE project_id = ? AND status IN ('VALIDATED', 'HUMAN_CERTIFIED')
            """,
