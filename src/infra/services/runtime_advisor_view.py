...
def format_runtime_advisory_text(advisory: dict) -> str:
    ...
    if providers:
        for p in providers:
            lines.append(f"- {p.get('name', 'unknown')}: available={p.get('available', False)} reason={p.get('reason', '')}")
    else:
    ...
    lines.extend([f"- {s}" for s in plan.get("signals", [])] or ["- none"])
    lines.append("Notes:")
    lines.extend([f"- {n}" for n in plan.get("notes", [])] or ["- none"])

    lines.append("")
    lines.append("Available Safe Actions:")
    if actions:
        for a in actions:
            lines.append(f"- {a.get('id', 'unknown')}: {a.get('label', '')} (confirm={a.get('requires_confirmation', False)}, mutates_config={a.get('mutates_config', False)})")
    else:
        lines.append("- none")
    ...
    if caps:
        ...
        lines.append(f"- explicit_confirmation_required_for_control: {caps.get('explicit_confirmation_required_for_control', True)}")
        for lim in caps.get("limitations", []) if isinstance(caps, dict) else []:
            lines.append(f"- limitation: {lim}")
        for blk in caps.get("blockers", []) if isinstance(caps, dict) else []:
