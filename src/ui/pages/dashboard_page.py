...
class DashboardPage(BasePage):
    def __init__(self, parent, controller):
        ...
        self.card_files = self._create_stat_card(self.grid_stats, "ðŸ“‚ Files Ingested", "0", 0)
        self.card_facts = self._create_stat_card(self.grid_stats, "ðŸ›ï¸ Facts Qualified", "0", 1)
        self.card_links = self._create_stat_card(self.grid_stats, "ðŸ”— Knowledge Links", "0", 2)
    ...
    def _update_ui_elements(self, f_count, fact_count, link_count, log_entries):
        ...
        if log_entries:
            ...
            for ts, label, status, diag in log_entries[:12]:
                 icon = "âœ”" if status in ('SUCCESS', 'COMPLETED') else "âœ–" if status == 'FAILED' else "âž¤"

                 # Display progress stages
                 if ":" in status:
                     main_status, stage = status.split(":", 1)
                     status_color = f"[{stage}]"
                 else:
                     status_color = "[DONE]" if status in ('SUCCESS', 'COMPLETED') else f"[{status}]"

                 log_text += f"{icon} {label.ljust(40)} {status_color}\n"

                 # Detailed Metrics or Errors
                 if diag:
                     try:
                         m = json.loads(diag)
                         if isinstance(m, dict):
                             if status == 'SUCCESS' and label.startswith("Extract:"):
                                 if "page_count" in m:
                                     metrics = f"  â””â”€ {m['page_count']} pages, {m['char_count'] // 1000}k chars, {m['image_count']} images, {m['block_count']} blocks"
                                     log_text += f"{metrics}\n"
                             elif status == 'FAILED':
                                 err_msg = m.get("error", "Unknown error")
                                 log_text += f"  â””â”€ Error: {err_msg[:80]}...\n"
                     except:
                         pass
