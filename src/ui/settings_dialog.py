...
class SettingsDialog(tk.Toplevel):
    ...
    def _build_runtime_advisor_tab(self):
        ...
        ttk.Button(actions_row, text="Clear Control Status", command=self._clear_runtime_control_status).pack(side=tk.RIGHT, padx=(0, 6))
    ...
    def _refresh_runtime_advisor(self):
        try:
            ...
            self.runtime_action_var.set(combo_values[0] if combo_values else "")
    ...
    def _run_runtime_advisor_action(self):
        ...
        if bool(selected.get("requires_confirmation", False)):
            confirmed = messagebox.askyesno(
                ...
                "This never installs/downloads/provisions. Runtime start/stop actions run only through explicit bounded control.",
            )
    ...
    def _build_governance_tab(self):
        ...
        try:
            ...
            logger.warning("Failed to load existing governance policies: %s", e)
        ...
        for role in ROLES:
            for disc in DISCIPLINES:
                ...
                for col_idx, domain in enumerate(DOMAINS):
                    ...
                    ttk.Checkbutton(matrix_frame, variable=var).grid(row=row_idx, column=col_idx + 1, padx=5, pady=2)
                row_idx += 1
        ...
        try:
            ...
            if hasattr(self.parent, "db") and self.parent.db:
                try:
                    ...
                    logger.error("Failed to save governance policies to DB: %s", e)
